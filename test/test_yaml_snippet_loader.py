...
class TestYamlSnippetLoader(object):
    ...
    def test_get_all_snippets(self):
        ...
        assert [{'log_i': 'this is snippet1!'}] in map(lambda x: x.run_section, s)
        assert [{'log_i': 'this is snippet2!'}] in map(lambda x: x.run_section, s)

    def test_get_snippet_by_name(self):
        s = self.yl.get_snippet_by_name('snippet2')
        assert s.name == 'snippet2'
        assert s.run_section == [{'log_i': 'this is snippet2!'}]
